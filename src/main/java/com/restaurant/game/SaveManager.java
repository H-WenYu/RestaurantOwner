...
public class SaveManager {
    ...
    public static boolean saveGame(Hotel hotel) {
        try {
            ...
            try (DataOutputStream dos = new DataOutputStream(
                ...
                // 版本号
                dos.writeInt(4); // 存档版本（v4增加菜品经验系统）
                ...
                // 已解锁菜品（新v4格式）
                Map<String, DishInstance> unlockedDishes = hotel.getUnlockedDishes();
                dos.writeInt(unlockedDishes.size());
                for (DishInstance di : unlockedDishes.values()) {
                    dos.writeUTF(di.getName());
                    dos.writeInt(di.getDishLevel());
                    dos.writeInt(di.getDishExp());
                }

                // 上架菜品
                List<String> activeNames = hotel.getActiveMenuNames();
                dos.writeInt(activeNames.size());
    ...
    public static Hotel loadGame() {
        ...
        try (DataInputStream dis = new DataInputStream(
            ...
            if (version < 4) {
                System.out.println("⚠️ 旧版存档不兼容（需要v4+），开始新游戏...");
                return null;
            }
            ...
            // 恢复已解锁菜品（v4格式）
            int dishCount = dis.readInt();
            Map<String, DishInstance> unlockedDishes = new LinkedHashMap<>();
            for (int i = 0; i < dishCount; i++) {
                String dishName = dis.readUTF();
                int dishLevel = dis.readInt();
                int dishExp = dis.readInt();
                Dish baseDish = createDishByName(dishName);
                if (baseDish != null) {
                    DishInstance instance = new DishInstance(baseDish, dishLevel, dishExp);
                    unlockedDishes.put(dishName, instance);
                    hotel.getMenu().add(baseDish); // 保留旧菜单兼容
                }
            }
            hotel.setUnlockedDishes(unlockedDishes);

            // 恢复上架菜品
            int activeCount = dis.readInt();
            List<String> activeNames = new ArrayList<>();
            for (int i = 0; i < activeCount; i++) {
                String activeName = dis.readUTF();
                activeNames.add(activeName);
                DishInstance instance = unlockedDishes.get(activeName);
                if (instance != null) {
                    instance.setActive(true);
                }
            }
            hotel.setActiveMenuNames(activeNames);
    ...
    private static Dish createDishByName(String name) {
        switch (name) {
            case "蛋炒饭":
                return Dish.createEggFriedRice();
            case "番茄炒蛋":
                return Dish.createTomatoEgg();
            case "麻婆豆腐":
                return Dish.createMaPoTofu();
            case "宫保鸡丁":
                return Dish.createKungPaoChicken();
            case "糖醋里脊":
                return Dish.createSweetSourPork();
            case "红烧肉":
                return Dish.createBraisedPork();
            case "清蒸鱼":
                return Dish.createSteamedFish();
            case "北京烤鸭":
                return Dish.createPekingDuck();
            case "龙虾":
                return Dish.createLobster();
            case "和牛牛排":
                return Dish.createWagyuSteak();
            default:
                return null;
        }
    }
     ...
     * 获取用户专属存档目录
     */
    private static String getUserSaveDirectory(String userId) {
        return SAVE_DIR + File.separator + "users" + File.separator + userId;
    }

    /**
     * 保存用户游戏
     */
    public static boolean saveGame(Hotel hotel, String userId) {
        if (userId == null || userId.isEmpty()) {
            return saveGame(hotel); // 兼容旧方法
        }
        try {
            String userDir = getUserSaveDirectory(userId);
            File dir = new File(userDir);
            if (!dir.exists()) {
                dir.mkdirs();
            }

            String savePath = userDir + File.separator + SAVE_FILE;
            return saveGameToPath(hotel, savePath);
        } catch (Exception e) {
            System.out.println("❌ 用户存档失败: " + userId + " - " + e.getMessage());
            return false;
        }
    }
     ...
     * 加载用户游戏
     */
    public static Hotel loadGame(String userId) {
        if (userId == null || userId.isEmpty()) {
            return loadGame(); // 兼容旧方法
        }
        String userDir = getUserSaveDirectory(userId);
        String savePath = userDir + File.separator + SAVE_FILE;
        return loadGameFromPath(savePath);
    }

    /**
     * 检查用户是否有存档
     */
    public static boolean hasSaveFile(String userId) {
        if (userId == null || userId.isEmpty()) {
            return hasSaveFile();
        }
        String userDir = getUserSaveDirectory(userId);
    ...
    private static boolean saveGameToPath(Hotel hotel, String savePath) {
        try (DataOutputStream dos = new DataOutputStream(
                new BufferedOutputStream(new FileOutputStream(savePath)))) {
    ...
    private static Hotel loadGameFromPath(String savePath) {
        File saveFile = new File(savePath);
        if (!saveFile.exists()) {
            return null;
        }
        try (DataInputStream dis = new DataInputStream(
            ...
            // 创建Hotel并恢复数据
            Hotel hotel = new Hotel(name, false);
            hotel.setMoney(money);
            hotel.setReputation(reputation);
            hotel.setHotelLevel(hotelLevel);
            hotel.setHotelExp(hotelExp);
